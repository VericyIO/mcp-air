...
export const registerAssessmentTools = (
  server: McpServer,
    ...
    async ({ assessmentPid }) => {
      try {
        return toolJsonResult(
          await api.listAssessmentInputArtifacts(assessmentPid),
        );
      } catch (error) {
  ...
  server.registerTool(
    ...
    async ({ projectPid, name, artifactPids }) => {
      try {
        return toolJsonResult(
          await api.createAssessmentDraft(projectPid, name, artifactPids),
        );
      } catch (error) {
  ...
  server.registerTool(
    ...
    async ({ projectPid, name, artifactPids }) => {
      try {
        return toolJsonResult(
          await api.startAssessment(projectPid, name, artifactPids),
        );
      } catch (error) {
