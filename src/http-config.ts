...
export const MCP_AIR_OAUTH_INTROSPECT_CLIENT_ID_ENV = 'OAUTH_INTROSPECT_CLIENT_ID' as const
...
export const loadMcpAirHttpRuntimeConfig = (
  ...
  const httpHost = env[MCP_AIR_HTTP_HOST_ENV]?.trim() || MCP_AIR_DEFAULT_HTTP_HOST
  const httpPort = parsePort(env[MCP_AIR_HTTP_PORT_ENV], MCP_AIR_DEFAULT_HTTP_PORT)
  const httpPath = env[MCP_AIR_HTTP_PATH_ENV]?.trim() || MCP_AIR_DEFAULT_HTTP_PATH

  const oauthIntrospectClientId = env[MCP_AIR_OAUTH_INTROSPECT_CLIENT_ID_ENV]?.trim()
  const oauthIntrospectClientSecret = env[MCP_AIR_OAUTH_INTROSPECT_CLIENT_SECRET_ENV]?.trim()
  ...
  return {
    ...
    httpPort,
    httpPath,
    ...(oauthIntrospectClientId !== undefined && oauthIntrospectClientId.length > 0
      ? { oauthIntrospectClientId }
      : {}),
    ...(oauthIntrospectClientSecret !== undefined && oauthIntrospectClientSecret.length > 0
